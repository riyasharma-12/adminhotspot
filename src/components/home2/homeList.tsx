...
import { fetchHomes, deleteHome } from "../../store/slices/homeSlice";
import { useNavigate } from "react-router-dom";
import WhyChooseList from "../whyChoose/WhyChooseList";
import MissionVisionList from "../mission/MVisionList";
...
const HomeList: React.FC = () => {
    ...
    const { homes, loading } = useSelector(
        (s: RootState) => s.homes
    ) as { homes: HomeItem[]; loading: boolean };
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-3">
                    {/* <Button
                        type="primary"
                        icon={<Plus />}
                        onClick={() => navigate("/dashboard/whyChooseList")}
                    >
                        Show Why Choose
                    </Button> */}
                ...
                {homes.map((item) => (
                    ...
                    >
                        ...
                        <div className="flex gap-4 mb-4">
                            {item.image1 && (
                                <Image src={item.image1} width={120} height={80} className="rounded-md object-cover" />
                            )}
                            {item.image2 && (
                                <Image src={item.image2} width={120} height={80} className="rounded-md object-cover" />
                            )}
                            {item.image3 && (
                                <Image src={item.image3} width={120} height={80} className="rounded-md object-cover" />
                            )}
                        </div>
                        ...
                        <div className="flex justify-end gap-3 mt-5">
                            <Button
                                icon={<Edit2 size={16} />}
                                onClick={() =>
                                    navigate("/dashboard/homeForm", { state: { home: item } })
                                }
                            >
                                Edit
                            </Button>
